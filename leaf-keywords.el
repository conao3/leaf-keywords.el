...
(defcustom leaf-keywords-after-conditions
  (leaf-list
   ...
   :ensure-system-package `(,@(mapcar (lambda (elm) `(unless (executable-find ,elm) (system-packages-install ,elm))) leaf--value) ,@leaf--body))
...
(defcustom leaf-keywords-normalize
  '(((memq leaf--key '())
    ...
    ((memq leaf--key '(:ensure-system-package))
     ;; Accept: 't, 'nil, symbol and list of these (and nested)
     ...
     ;;         remove duplicate element
     (let ((ret (leaf-flatten leaf--value)))
       (mapcar 'symbol-name (delete-dups (delq nil (leaf-subst t leaf--name ret))))))
...
(defmacro leaf-keywords-handler-ensure-system-package (pkg)
  "Handler :ensure-system-package for PKG."
  `(unless `(executable-find ,(symbol-name pkg))
     (system-packages-install ,(symbol-name pkg))))
